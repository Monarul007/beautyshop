    ...
    <section class="category-section">
		<div class="container">
			...
			<div class="page-top-info">
				<div class="breadcrumb"> &nbsp;<a href="{{ route('home') }}">Home&nbsp;</a> » &nbsp;{{$brands->name}}&nbsp</div>
			</div>
            <div class="row">
                <div class="col-md-5">
                    <img src="/images/brands/{{$brands->image}}" alt="">                
                </div>
                <div class="col-md-7">
                    <h3>{{$brands->name}}</h3>
                    <p>{{$taglessdesc}}</p>
			...
			<div class="row">
							...
							<div class="card">
								...
								<div class="card-header bg-light p-0" id="heading{{$cat->id}}">
									<h2 class="mb-0" style="line-height: 1;">
										<a href="{{$cat->url}}" class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapse{{$cat->id}}" aria-expanded="true" aria-controls="collapse{{$cat->id}}">{{$cat->name}}</a>
									</h2>
								</div>
								<div id="collapse{{$cat->id}}" class="collapse" aria-labelledby="heading{{$cat->id}}" data-parent="#showCat">
									<div class="card-body">
										...
										@foreach($cat->categories as $subcat)
											<li class="list-group-item"><a href="{{$subcat->url}}">{{$subcat->name}}</a></li>
										@endforeach
										</ul>
									</div>
								</div>
							...
							<li>
								...
								<div class="lp-item">
									...
									<div class="lp-content">
										<a href="http://localhost:8000/products/{{$latest->id}}" class="title">{{$latest->name}}</a>
				...
				<div class="col-lg-9  order-1 order-lg-2 mb-5 mb-lg-0">
						...
						<div class="row">
							...
							<div class="col-lg-4 col-md-4 hide-m">
								<div class="filter__found">
									<h6><span>{{$prodCount}}</span> Products found</h6>
								</div>
							</div>
					...
					<div class="row display mb-5">
						...
						<div class="col-lg-4 col-sm-4 col-6">
							<div class="single-product">
								<div class="product-img">
									...
									<a href="/products/{{$brand->id}}"><img src="/images/products/{{$brand->product_img}}" alt=""></a>
								...
								<div class="product-info clearfix">
									<div class="fix">
										<p class="floatright hidden-sm">{{$brand->bname}}</p>
										<h4 class="post-title text-left"><a href="/products/{{$brand->id}}">{{$brand->name}}</a></h4>
									</div>
									<div class="fix">
										<span class="pro-price text-left">£ {{$brand->after_pprice}} <span>£ {{$brand->before_price}}</span></span>
						...
						@endforeach
						<div class="m-auto paginator">{{$brandDatas->links()}}</div>
					</div>
				</div>
			</div>
		</div>
