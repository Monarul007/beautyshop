        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">View Canceled</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="card">
              ...
              <!-- /.card-header -->
              <div class="card-body">
                <table id="canceled" class="table table-bordered table-hover">
                  <thead>
                  <tr>
                  ...
                  <tbody>
                    @foreach($getCanceled as $canceled)
                    <tr>
                        <td><input type="checkbox" class="slct" value="{{$canceled->order_number}}"></td>
                        <td><a href="/admin/get_invoice/{{$canceled->order_number}}">{{$canceled->order_number}}</a></td>
                        <td>{{$canceled->name}}</td>
                        <td>{{$canceled->email}}</td>
                        <td>{{$canceled->grand_total}}</td>
                        <td>{{$canceled->cancel_date}}</td>
                        <td><a href="/admin/get_invoice/{{$canceled->order_number}}"><i class="fa fa-eye"></i> View</a></td>
                    </tr>
                    @endforeach
                  </tbody>
                </table>
    ...
    $('#refresh').click(function(){
      ...
      $('#selectAction').change(function(){
        ...
        if(take_value == "confirm"){
          ...
          $.ajax({
              url: "{{ URL::route('admin.orders.confirm_canceled') }}",
              method: 'post',
              data: formData,
              contentType: false,
              cache: false,
